...
def _dataset(model, n, p, rho, block, seed, extra, x_model=None):
    x_model = x_model or model
    Sigma = make_block_cov(p, rho=rho, block_size=block)
    X = _sample(x_model, n, p, np.eye(p), seed, extra)
    Y = _sample(model,   n, p, Sigma,     seed + 10**6, extra)
    return X, Y

def run_once(model="gaussian", p=250, n=80, rho=0.30, block=20, seed=0, extra=None, winsorize=None, B_list=None, n_jobs=-1, x_model=None):
    t0 = time.perf_counter()
    X, Y = _dataset(model, n, p, rho, block, seed, extra or {}, x_model=x_model)
    ...
    row = {"model": model, "x_model": x_model or model,
           "p": p, "n": n, "rho": rho, "block": block, "seed": seed}
...
def main():
    ...
    ap.add_argument("--x-model", type=str, default=None,
                    help="Marginal for group X. Default: same as --models entry.")
    args = ap.parse_args()
            ...
            for rho in rho_list:
                ...
                for r in range(args.reps):
                    rows.append(run_once(model=model, p=args.p, n=n, rho=rho, block=20, seed=r,
                                         extra=extra, winsorize=args.winsorize, B_list=B_list,
                                         n_jobs=n_jobs, x_model=args.x_model))
