...
**Author:** Hrishikesh Deepak Dhole (MS Data Science, Seattle University)
...
Given two independent samples `X ∈ R^{n1×p}` and `Y ∈ R^{n2×p}`, we test edge-level equality of correlations

H_{0,ij}: ρ_{ij,1} = ρ_{ij,2} for all i < j

while controlling the false discovery rate at level α. Four procedures are implemented and compared:
...
Simulations cover Gaussian, t₆, Laplace, and Exp(1) marginals across sample sizes n₁ = n₂ ∈ {60, 80, 120}, dimensions p ∈ {250, 500, 1000}, and multiple block-correlation structures (simple block, block-AR(1), block-decay).
...
```
3. Open the analysis notebooks in Jupyter and run them top-to-bottom. Each notebook's first cell locates `results/tables/` automatically.
...
- **Null definition (Patch 2).** Non-Gaussian null runs draw both groups from the same marginal family with identity covariance, matching Cai & Liu's H₀ definition. Prior mixed-marginal behavior (X Gaussian regardless of Y) is preserved as an ablation via `--x-model gaussian`.
- **Threshold selection (Patch 1).** All three implementations (LCT-N, LCT-B, LCT-B v2) return the infimum of `{t : est_FDR(t) ≤ α}` per Cai & Liu Eq. (9), by ascending scan with early return.
- **Bootstrap defaults.** `B=100` for p ≈ 250; `B=200` for p ≥ 500 or non-Gaussian marginals with noisy null control. Cai–Liu variance estimator by default; light winsorize (5) recommended for heavy-tailed data. Locked in `results/defaults.json` via `scripts/make_defaults.py`.
...
- `patch02_null_calibration.md` — matching-marginal H₀ by default
...
Hrishikesh Dhole — hrishikeshdhole0@gmail.com